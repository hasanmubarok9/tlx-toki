...
int main() {
  ...
  int pos = k - 1;
  cout << "nilai pos: " << pos << endl;
  long long totalComb = 1;
  for (int i = 0; i < pos; i++) {
    totalComb *= 3LL;
  } // 3 ^ (k - 1)
  cout << "nilai totalComb: " << totalComb << endl;
  ...
  for (long long mask = 0; mask < totalComb; mask++) {
    ...
    for (int i = 0; i < pos; i++) {
      ...
      if (op == 0) { // concat
        cout << "masuk concat, nilai num: " << num << ", dan nilai d: " << d
             << endl;
        num = num * 10 + d;
        cout << "nilai num yang baru: " << num << endl;
      } else { // finish current number
        cout << "masuk finish current number, nilai sign: " << sign
             << ", dan nilai num: " << num << endl;
        total += sign * num;
      ...
      cout << "akhir dari i: " << i << ", nilai d: " << d
           << ", dan nilai num: " << num << endl;
    }
    cout << "nilai sign: " << sign << ", dan nilai num: " << num
         << ", nilai sign * num: " << sign * num << endl;
