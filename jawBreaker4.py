...
def lookingForTheBiggest(m, n, matrices):
    allPossibleIndexPicked = []

    # flag matrices, to mark the point
    flagMatrices = [[False if matrices[i][j] != '.' else True for j in range(n)] for i in range(m)]
    ...
    while not allTrue(flagMatrices):
        ...
        if(len(picked) > 1):
            allPossibleIndexPicked.append(picked)

    return allPossibleIndexPicked
...
if __name__ == "__main__": 
    ...
    for collection in allPossibleSetIndexPicked:
        ...
        # first attempt to demolished
        runtuhkan(newMatrix, m, n)
        ...
        if len(allPossibleSetIndexPickedAfterDemolished) > 0:
            secondVal = len(max(allPossibleSetIndexPickedAfterDemolished, key=len))
            secondValue = secondVal * (secondVal-1)
