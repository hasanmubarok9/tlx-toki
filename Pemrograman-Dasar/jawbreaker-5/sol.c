...
int isFinish(int arr[30][30], int N, int M) {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < M; j++) {
      ...
      for (int k = 0; k < 4; k++) {
        int x_new = i + move_x[k];
        int y_new = j + move_y[k];

        if (x_new < 0 || x_new >= N || y_new < 0 || y_new >= M) continue;
        if (arr[x_new][y_new] == arr[i][j]) return 0;
      }
    }
  }
...
int two_click(int arr[30][30], int N, int M) {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < M; j++) {
      ...
      if ((visited[i][j] == 0) && map[i][j] != 0 && color != 0) {
        ...
        if (isFinish(map, N, M)) return costA*(costA - 1);
        costB = two_click(new_map, N, M);
